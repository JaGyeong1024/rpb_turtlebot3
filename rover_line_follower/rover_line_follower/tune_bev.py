...
class BEVTuner(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'토픽이 보이지 않으면 다음을 확인하세요:')
        self.get_logger().info(f'  1. ros2 topic list 로 토픽 확인')
        self.get_logger().info(f'  2. ros2 topic hz {camera_topic} 로 이미지 발행 확인')
        
        self._latest_image = None
        self._bev_matrix = None
        self._bev_size_locked = False  # BEV 크기 고정 여부
        self._initial_bev_width = 640  # 카메라 이미지 크기에 맞춤 (640x480)
        self._initial_bev_height = 480
        ...
        self._bev_width = 640  # 카메라 이미지 크기에 맞춤 (640x480)
        self._bev_height = 480
        ...
        # BEV 이미지 크기 조정 (최대값을 카메라 해상도보다 크게 설정)
        cv2.createTrackbar('BEV_Width', 'BEV Tuner', self._bev_width, 1280, 
                          lambda x: self._update_bev_size(x, None))
        cv2.createTrackbar('BEV_Height', 'BEV Tuner', self._bev_height, 960, 
    ...
    def _check_and_display(self):
        ...
        if self._latest_image is None:
            ...
            cv2.imshow('BEV Tuner', dummy_img)
            cv2.imshow('BEV Result', dummy_img)
            return
        ...
        if self._bev_matrix is not None:
            ...
            # BEV 행렬이 아직 계산되지 않았으면 원본 이미지만 표시
            cv2.imshow('BEV Tuner', self._latest_image)
            dummy_result = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.putText(dummy_result, 'Calculating BEV matrix...', (50, 240),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            cv2.imshow('BEV Result', dummy_result)
    ...
    def _display_images(self):
        ...
        # 키 입력은 _check_and_display에서 처리됨
...
def main(args=None):
    ...
    try:
        ...
        if node._bev_matrix is not None:
        node._print_matrix()
        cv2.destroyAllWindows()
        if node is not None:
        node.destroy_node()
